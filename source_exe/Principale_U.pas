...
uses
  ...
  DBTables, ComCtrls, StdCtrls, ADODB,StrUtils, jpeg, DBCtrls, Mask,ComObj,ShellAPI;
...
type
  TFPrincipale = class(TForm)
    ...
    procedure Panel28MouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure Panel28MouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure Panel29MouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure Panel29MouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
  ...
  private
    ...
    procedure ImportAN_Access(destination:TADOTable;source:TADOQuery;tableName:string);
    procedure EnleveBlancChamp(Table_name:string);
  public
    { Public-déclarations }
    chem : string;
    procedure Init;
...
uses journal, Journaux, Groupe,   EtatCourant_U, Guidabon, Daspecia,
  ...
  LstAuxi, Impot_Synth_U,BalConsolidee_U, VueConsolide_U, Journaux_U,
  EtatBalance_U;
...
begin
  ...
  Special.Guidabon.FieldByName('Periodede').DisplayLabel:='Période de..';
  Special.Guidabon.FieldByName('Periodea').DisplayLabel:='Période à..';
  Special.Guidabon.FieldByName('Jieme').DisplayLabel:='ième Jour';
  Special.Guidabon.FieldByName('Mieme').DisplayLabel:='ième Mois';
  Special.Guidabon.FieldByName('Frequence').DisplayLabel:='Fréquence';
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if not hita then Showmessage('Déjà ouvert');
    end;
  end;
end;
...
begin
  ...
  if not hita then Showmessage('Cette période n''est jamais ouverte');
end;
  ...
  begin
    ...
    begin
      ...
      mr:=MessageDlg('Le compte n°'+Special.ToutEltJrnCompte.value+
                   ' est utilisé dans'+ Special.ToutEltJrnJournal.value+' mais non créé. Voulez-vous le créer maintenant ? ',
...
begin
  AsehoyEcriture(0);
  FInformat.Asehoy('Renumerotation des pièces ','PATIENTEZ....',1000);
  DASQL.SelJournal.close;
  DASQL.SelJournal.sql.clear;
  DASQL.SelJournal.sql.Add('select * from Journal  ');
...
begin
  ...
  begin
    ...
    if mtdebit<>mtcredit then showmessage('ecriture non soldé'+special.JournalCode.Value);
...
begin
  ...
  begin
    ...
    if mtdebit<>mtcredit then showmessage('ecriture non soldé'+special.JrResumCode.Value);
...
begin
  ...
  begin
    if length(Special.JournalPeriode.Value)<>7 then
      ShowMessage('Longeur période:'+dASQL.ResumeJournal.Value);
    if length(Special.JournalCodeJrnx.Value)=0 then
      ShowMessage('Vérifier l''existence du journal:'+
...
begin
  ...
  begin
    ...
    if mtdebit<>mtcredit then showmessage('ecriture non soldé'+special.JrFinReCode.Value);
...
begin
  ...
  begin
    ...
    begin
      ...
      if DASQL.SelNumCpt.eof then
      begin
        valeur:=inputbox('Vérification','Le compte dans la zone tiers du '+Special.JournalCode.Value+
...
begin
  ...
  Cmpt_b := InputBox('Compta', 'Compte à :', 'Z');
...
begin
  ...
  showMessage('Conversion en Ariary Terminé');
end;
...
begin
  ...
  showMessage('Conversion en Fmg Terminé');
end;
...
begin
  FInformat.Asehoy('Opération en cours...','Patienter s.v.p',100);
...
begin
  MenuG_NonVisible;
  AsehoyAnneeDeclare(Edit1.text);
  AsehoyCanevaDCOMM(Edit1.text, false);
end;
...
begin
  mrConfirm:=MessageDlg('Demande de confirmation,'+
      'Voulez-vous vraiement Renuméroter ?',MtConfirmation,[mbYes,mbNo],0);
  case mrConfirm of
    mrNo   : Exit;
    mrYes  :;
  end;
...
begin
  ...
  try
    DaConnect.ChangeCheminConnection(DaConnect.C_Temporaire,Special.param_serverip.Value,'postgres');
    DaConnect.Operation_temp.Close;
    DaConnect.Operation_temp.SQL.Clear;
    DaConnect.Operation_temp.SQL.Add('CREATE DATABASE '+Edit3.Text+' TEMPLATE '+Edit2.Text);
    DaConnect.Operation_temp.ExecSQL;
    showMessage('CREATION BDD '+Edit3.Text+' OK');
  except
    on E:Exception do
    begin
      Memo1.Text := DaConnect.C_Temporaire.ConnectionString;
      ShowMessage('ERREUR CREATION BDD '+E.Message);
    end;
  end;
  try
    DaConnect.ChangeCheminConnection(DaConnect.C_Temporaire,Special.param_serverip.Value,Edit3.text);
    DaConnect.Operation_temp.Close;
    DaConnect.Operation_temp.SQL.Clear;
    DaConnect.Operation_temp.SQL.Add('DELETE FROM eltjrn;');
    DaConnect.Operation_temp.SQL.Add('DELETE FROM journal;');
    DaConnect.Operation_temp.SQL.Add('DELETE FROM lettrage;');
    DaConnect.Operation_temp.ExecSQL;
    showMessage('VIDANGE BDD OK');
  except
    on E:Exception do
    begin
      Memo1.Text := DaConnect.C_Temporaire.ConnectionString;
      ShowMessage('ERREUR VIDANGF BDD '+E.Message);
    end;
  end;
  Societe(0);
end;
...
procedure TFPrincipale.Label27MouseEnter(Sender: TObject);
begin
  Label27.Font.Color := clRed
end;

procedure TFPrincipale.Label27MouseLeave(Sender: TObject);
begin
  Label27.Font.Color := clWhite
end;

procedure TFPrincipale.Label5MouseEnter(Sender: TObject);
begin
  Label5.Font.Color := clRed

end;

procedure TFPrincipale.Label5MouseLeave(Sender: TObject);
begin
  Label5.Font.Color := clWhite
end;

procedure TFPrincipale.Label8MouseEnter(Sender: TObject);
begin
  Label8.Font.Color := clRed
end;

procedure TFPrincipale.Label8MouseLeave(Sender: TObject);
begin
  Label8.Font.Color := clWhite
end;

procedure TFPrincipale.Label15MouseLeave(Sender: TObject);
begin
  Label15.Font.Color := clWhite
end;

procedure TFPrincipale.Label6MouseEnter(Sender: TObject);
begin
  Label6.Font.Color := clRed

end;

procedure TFPrincipale.Label6MouseLeave(Sender: TObject);
begin
  Label6.Font.Color := clWhite
end;

procedure TFPrincipale.Label15MouseEnter(Sender: TObject);
begin
  Label15.Font.Color := clRed
end;

procedure TFPrincipale.SpeedButton2Click(Sender: TObject);
begin
  //AsehoyAnneeDeclare(Edit1.text);
end;

procedure TFPrincipale.Label2Click(Sender: TObject);
var param,Appli,ndf:string;
begin
  if MessageDlg('Sauvegarde de base de données, Cliquez sur le bouton "Yes"',mtConfirmation,[mbYes,mbNo],0)<>mrYes then
    exit;
  ndf:=FormatDateTime('yymmdd',date())+'_'+DaConnect.C_Document.DefaultDatabase+'_'+FormatDateTime('hhnn',time());
  Appli:='"C:\Program Files (x86)\PostgreSQL\9.4\bin\pg_dump.exe"';
  param:='--host '+DBEdit1.EditText+' --port 5432 --username "postgres" --no-password  --format custom --blobs --verbose '+
         '--file '+'"D:\Sauvegarde BDD\'+ndf+'.backup" "'+DaConnect.C_Document.DefaultDatabase+'"';
  InputBox('Commande','Commande', Appli+' '+param);
  ShellExecute(Handle,'Open', PChar(Appli), PChar(param), nil,SW_SHOW);
end;

procedure TFPrincipale.Label2MouseEnter(Sender: TObject);
begin
  Label2.Font.Color := clRed

end;

procedure TFPrincipale.Label2MouseLeave(Sender: TObject);
begin
  Label2.Font.Color := clWhite
end;

procedure TFPrincipale.Label5Click(Sender: TObject);
begin
  MenuG_NonVisible;
  AsehoyJournaux(0);
end;

procedure TFPrincipale.Panel13MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFPrincipale.Panel13MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFPrincipale.Panel4MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFPrincipale.Panel4MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel4);
end;

procedure TFPrincipale.Panel14MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseUp(Panel14);
  MenuG_NonVisible;
...
procedure TFPrincipale.Panel14MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel14);
end;

procedure TFPrincipale.Panel15MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFPrincipale.Panel15MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel15);
end;

procedure TFPrincipale.Panel16MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFPrincipale.Panel16MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel16);
end;

procedure TFPrincipale.ImportAN_Access(destination:TADOTable;source:TADOQuery;tableName:string);
var j:integer;
  FieldName:string;
begin
  if MessageDlg('Import AN ancien logiciel',mtConfirmation,[mbYes,mbNo],0)= mrYes then
  begin
    destination.Close;
    ...
    begin
      destination.Insert;
      for j:=0 to Special.Journal.FieldCount -1 do
      begin
        FieldName := lowerCase(source.Fields[j].FieldName);
        if (FieldName <> 's_collineage') and  (FieldName <> 's_generation') and (FieldName <> 's_guid') and  (FieldName <> 's_lineage') then
          if (FieldName='montant') and (lowerCase(tableName)='eltjrn') then
            destination.FieldValues[FieldName]:= Round(source.Fields[j].value)
          else
            destination.FieldValues[FieldName]:= source.Fields[j].value;
      end;
      destination.Post;
      source.Next
    end;
...
begin
  ...
  if DaConnect.C_Document.DefaultDatabase = 's2102223'  then
    ADOConnection1.DefaultDatabase := 'Z:\S2102223\S2102223'
  else if DaConnect.C_Document.DefaultDatabase = 's3112223' then
    ADOConnection1.DefaultDatabase := 'Z:\S3112223\S3112223'
  else
    exit;

  QuerySource.Close;
  QuerySource.SQL.Clear;
  QuerySource.SQL.Add('SELECT *');
  QuerySource.SQL.Add('FROM Journal');
  QuerySource.SQL.Add('WHERE  Left(Code,2)=''AN''');
  QuerySource.Open;
  ImportAN_Access(special.Journal,QuerySource,'journal');

  QuerySource.Close;
  QuerySource.SQL.Clear;
  QuerySource.SQL.Add('SELECT *');
  QuerySource.SQL.Add('FROM eltjrn');
  QuerySource.SQL.Add('WHERE  Left(journal,2)=''AN''');
  QuerySource.Open;
  ImportAN_Access(special.EltJrn,QuerySource,'eltjrn');
  }
end;

procedure TFPrincipale.EnleveBlancChamp(Table_name:string);
var i:word;
begin
  PStockee.ADOTable1.Close;
  PStockee.ADOTable1.TableName := Table_name;
  PStockee.ADOTable1.Open;
  for i:=0 to PStockee.ADOTable1.FieldCount -1 do
  begin
    if (PStockee.ADOTable1.Fields[i].DataType in [ftString, ftFixedChar, ftWideString]) then
    begin
      try
        PStockee.Operation2.SQL.Clear;
        PStockee.Operation2.SQL.Add('UPDATE '+table_Name);
        PStockee.Operation2.SQL.Add('SET '+PStockee.ADOTable1.Fields[i].fieldName+
                                      '= TRIM(TRAILING FROM '+PStockee.ADOTable1.Fields[i].fieldName+')');
        PStockee.Operation2.ExecSQL;
      except
      end;
    end;
  end;
...
procedure TFPrincipale.Label12Click(Sender: TObject);
begin
  EnleveBlancChamp('journal');
  EnleveBlancChamp('eltjrn');
end;

procedure TFPrincipale.Label13Click(Sender: TObject);
begin
  if MessageDlg('Voulez-vous vraiement initialise AN?', mtConfirmation,[mbYes,mbNo],0)=mrYes then
  try
    PStockee.Operation2.SQL.Clear;
    PStockee.Operation2.SQL.Add('DELETE FROM eltjrn WHERE left(journal,2)=''AN'';');
    PStockee.Operation2.SQL.Add('DELETE FROM journal WHERE left(code,2)=''AN''');
    PStockee.Operation2.ExecSQL;
  except
    On E:Exception do
...
procedure TFPrincipale.Label13MouseEnter(Sender: TObject);
begin
  Label13.Font.Color := clRed
end;

procedure TFPrincipale.Label12MouseEnter(Sender: TObject);
begin
  Label12.Font.Color := clRed
end;

procedure TFPrincipale.Label3MouseEnter(Sender: TObject);
begin
  Label3.Font.Color := clRed
end;

procedure TFPrincipale.Label3MouseLeave(Sender: TObject);
begin
  Label3.Font.Color := clWhite

end;

procedure TFPrincipale.Label12MouseLeave(Sender: TObject);
begin
  Label12.Font.Color := clWhite
end;

procedure TFPrincipale.Label13MouseLeave(Sender: TObject);
begin
  Label13.Font.Color := clWhite
end;

procedure TFPrincipale.Label32Click(Sender: TObject);
begin
  MenuG_NonVisible;
  AsehoyAnneeDeclare(Edit1.text);
  AsehoyCanevaDCOMM(Edit1.text,true);
end;

procedure TFPrincipale.Label32MouseEnter(Sender: TObject);
begin
  Label32.Font.Color := clRed

end;

procedure TFPrincipale.Label32MouseLeave(Sender: TObject);
begin
  Label32.Font.Color := clWhite
end;
...
procedure TFPrincipale.Panel28MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel18);
end;

procedure TFPrincipale.Panel28MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseUp(Panel18);
  MenuG_NonVisible;
...
procedure TFPrincipale.Panel29MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel17);
end;

procedure TFPrincipale.Panel29MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  Special.PrepCons.Close;
...
procedure TFPrincipale.Image4MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel14);
end;

procedure TFPrincipale.Image4MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseUp(Panel14);
  MenuG_NonVisible;
...
procedure TFPrincipale.Image2MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel13);
end;
...
procedure TFPrincipale.Image3MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel4);
end;
...
procedure TFPrincipale.Image5MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel15);
end;

procedure TFPrincipale.Image5MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  Special.GranLivr.Close;
  PanelMouseUp(Panel15);
...
procedure TFPrincipale.Image6MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel16);
end;

procedure TFPrincipale.Image6MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  Special.PrepCons.Close;
  PanelMouseUp(Panel16);
...
procedure TFPrincipale.Image7MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  PanelMouseDown(Panel17);
end;

procedure TFPrincipale.Image7MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  Special.PrepCons.Close;
  PanelMouseUp(Panel17);
...
procedure TFPrincipale.Label35MouseEnter(Sender: TObject);
begin
  Label35.Font.Color := clRed

end;

procedure TFPrincipale.Label35MouseLeave(Sender: TObject);
begin
