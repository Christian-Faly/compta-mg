...
uses USQL, UPStocke, DaCommun, ImpLettr, UEtaRembTVA, Daspecia;
...
begin
  ...
  if not CheckBox1.Checked then // Pas de report à nouveau
    PStockee.RRembTVA.SQL.Add('and((JrTVA<''AN'')or(JrTVA>''ANZ''))');
  if  CheckBox3.Checked then // Pas de règlement
...
begin
   AsehoyLettrage(0,false,Special.param_serverip.Value,Commun.SocieteCode.Value,PStockee.RRembTVATiers.Value);
...
begin
  AsehoyLettrage(0,false,Special.param_serverip.Value,Commun.SocieteCode.Value,PStockee.Q_RembTVAByLigneCompteFnrs.Value);
end;
